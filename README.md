...
### 6. コマンドブロック対応システム（新機能）

#### 座標指定による物理オブジェクト生成
- **コマンドブロック実行**: `/physxmc ball` と `/physxmc ramp` コマンドをコマンドブロックから実行可能
- **座標指定**: コマンドブロック実行時は座標パラメータ（x, y, z）で生成位置を指定
- **自動化対応**: レッドストーン回路との組み合わせによる自動物理オブジェクト生成
- **プレイヤー不要**: プレイヤーがいない場所でも物理オブジェクトを生成可能

#### 対応コマンド
- **球体生成**: `/physxmc ball <半径> <密度> <x> <y> <z> [マテリアル]`
- **ランプ生成**: `/physxmc ramp create <角度> <幅> <長さ> <厚み> <x> <y> <z> [ブロック名]`
...
# コマンドブロック用: /physxmc ball <半径> <密度> <x> <y> <z> [マテリアル]

# ランプシステム
+/physxmc ramp create <角度> <幅> <長さ> <厚み> [ブロック名]
# 例: /physxmc ramp create 20 3 6 0.5 QUARTZ_BLOCK
# コマンドブロック用: /physxmc ramp create <角度> <幅> <長さ> <厚み> <x> <y> <z> [ブロック名]
+/physxmc ramp remove
+/physxmc ramp clear
+/physxmc ramp count
...
### 1.5-SNAPSHOT (最新版 - 実装済み)
- 🆕 **コマンドブロック対応システム**: `/physxmc ball`と`/physxmc ramp`コマンドをコマンドブロックから実行可能
- 🎯 **座標指定生成**: コマンドブロック実行時は座標パラメータで生成位置を指定
- 🔧 **自動化対応**: レッドストーン回路との組み合わせによる自動物理オブジェクト生成
- 📉 **loadコマンド削除**: 非同期エンティティ追加エラー対策のため`/physxmc load`コマンドを削除
