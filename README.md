...
# 球体システム（コマンドブロック用）
+/physxmc ball <半径> <密度> <x> <y> <z> [マテリアル]
# 例: /physxmc ball 1.5 2.0 100 70 -200 MAGMA_BLOCK
...
# ランプシステム（コマンドブロック用）
+/physxmc ramp create <角度> <幅> <長さ> <厚み> <x> <y> <z> [ブロック名]
# 例: /physxmc ramp create 20 3 6 0.5 100 70 -200 QUARTZ_BLOCK
...
#### `/physxmc ball <半径> <密度> [マテリアル]` (プレイヤー用)
- **機能**: プレイヤーの目の位置に転がる球体を召喚
- **半径**: 球体の半径（ブロック数、正の小数可）
- **密度**: 球体の密度（正の小数、重さに影響）
- **マテリアル**: 球体の表示素材（省略時はスライムブロック）
- **例**: `/physxmc ball 1.5 2.0 MAGMA_BLOCK`

#### `/physxmc ball <半径> <密度> <x> <y> <z> [マテリアル]` (コマンドブロック用)
- **機能**: 指定座標に転がる球体を召喚
- **半径**: 球体の半径（ブロック数、正の小数可）
- **密度**: 球体の密度（正の小数、重さに影響）
- **x, y, z**: 召喚する座標（ワールド座標）
- **マテリアル**: 球体の表示素材（省略時はスライムブロック）
- **例**: `/physxmc ball 1.0 3.0 100 70 -200 SLIME_BLOCK`

### ランプシステム専用コマンド

傾斜板（斜面）をワールドに設置してオブジェクトを転がすことができます。

#### `/physxmc ramp create <角度> <幅> <長さ> <厚み> [ブロック名]` (プレイヤー用)
| パラメータ | 説明 |
|-----------|------|
...
#### `/physxmc ramp create <角度> <幅> <長さ> <厚み> <x> <y> <z> [ブロック名]` (コマンドブロック用)
| パラメータ | 説明 |
|-----------|------|
| 角度 | ピッチ方向の傾斜角度（度数、正の値で目線方向に下がる） |
| 幅 | X方向のサイズ（ブロック数、正の値） |
...
**例**: `/physxmc ramp create 15 4 8 0.3 100 70 -200 STONE`
...
#### 自動球体生成システム
```bash
# 自動ボール発射台（リピート型コマンドブロック）
/physxmc ball 0.8 2.5 ~ ~2 ~3 MAGMA_BLOCK

# 段階的な球体生成（チェーン型コマンドブロック）
/physxmc ball 1.0 1.0 ~-2 ~ ~ SLIME_BLOCK
/physxmc ball 1.5 3.0 ~ ~ ~ IRON_BLOCK
/physxmc ball 2.0 5.0 ~2 ~ ~ OBSIDIAN
```

#### 自動ランプ生成システム
```bash
# 自動スロープ建設（レッドストーン信号で実行）
/physxmc ramp create 15 5 10 0.5 100 70 -200 QUARTZ_BLOCK

# 連続ランプシステム（チェーン型コマンドブロック）
/physxmc ramp create 10 3 5 0.3 ~ ~ ~5
/physxmc ramp create 15 3 5 0.3 ~ ~ ~10
/physxmc ramp create 20 3 5 0.3 ~ ~ ~15
```

#### レッドストーン連携
- **プレッシャープレート**: プレイヤーが踏むと自動で球体やランプを生成
- **レッドストーンタイマー**: 定期的な自動オブジェクト生成
- **検知システム**: プレイヤーの動きに応じた動的な物理環境構築
...
- **自動工場**: コマンドブロックによる自動物理オブジェクト生産システム
- **動的建築**: 位置を指定した精密な物理構造の自動構築
